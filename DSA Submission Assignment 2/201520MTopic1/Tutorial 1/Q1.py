...
def minmax(Data):
    ...
    for i in range(0, len(Data)): # for i in Data (alternative soln)
        #1 being the fist element, len(Data) being the  last number of iteration
        if Data[i] < small: # compares the data wherether it is smaller than the number
            small = Data[i] #if its smaller than the number, save
        elif Data[i] > big: # compares the data whether it is larger than the number
            big = Data[i] # if its larger save the data
